...
export const ResumeUploader: React.FC<ResumeUploaderProps> = ({ onUpload, isLoading, error }) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full p-8 text-center" onDragEnter={handleDrag}>
      ...
      <p className="text-lg text-gray-400 mb-8 max-w-2xl">
        Upload your resume to begin a personalized interview session. Our AI will generate relevant questions based on your experience.
      </p>
      ...
      <div className="w-full max-w-lg">
        ...
        <div className="mt-6 flex flex-row space-x-4">
          <button
            ...
            className="flex-1 flex items-center justify-center px-6 py-3 bg-cyan-500 text-white font-bold rounded-full shadow-lg transition-all transform hover:scale-105 disabled:bg-gray-600 disabled:cursor-not-allowed disabled:scale-100"
          >
            {isLoading ? (
              <>
                <Spinner />
          ...
          <button
            onClick={() => console.log('Check ATS Score clicked')}
            disabled={!file || isLoading}
            className="flex-1 flex items-center justify-center px-6 py-3 bg-cyan-500 text-white font-bold rounded-full shadow-lg transition-all transform hover:scale-105 disabled:bg-gray-600 disabled:cursor-not-allowed disabled:scale-100"
          >
            Check ATS Score
